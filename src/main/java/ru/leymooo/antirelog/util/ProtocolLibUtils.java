            ...
            public void onPacketSending(PacketEvent event) {
                ...
                for (CooldownType cooldownType : types) {
                    if (material == cooldownType.getMaterial()) {
                        ...
                        boolean hasCooldown = cooldownManager.hasCooldown(event.getPlayer(), cooldownType, cooldownMs);
                        if (hasCooldown) {
                            long remaining = cooldownManager.getRemaining(event.getPlayer(), cooldownType, cooldownMs);
                            if (Math.abs(remaining - durationMs) > 100) {
                                if (!pvPManager.isPvPModeEnabled() || pvPManager.isInPvP(event.getPlayer())) {
                                    if (duration == 0) {
                                        event.setCancelled(true);
                                        return;
                                    }
                                    event.getPacket().getIntegers().write(0, (int) Math.ceil(remaining / 50.0));
                                }
                            }
                        }
                    }
